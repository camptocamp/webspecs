...
class AccumulatingRequest6[-In,+T1,+T2,+T3,+T4,+T5,+T6,+Out](
  ...
  override def assertPassed(in: In)(implicit context: ExecutionContext, uriResolvers:UriResolver):AccumulatedResponse6[T1,T2,T3,T4,T5,T6,Out] =
...
def trackThen [A,B] (next: Response[Out] => Request[Out,A]):AccumulatingRequest7[In,T1,T2,T3,T4,T5,T6,Out,A] =
  ...
  def execute(in: In)(implicit context: ExecutionContext, uriResolvers:UriResolver):AccumulatedResponse6[T1,T2,T3,T4,T5,T6,Out] = {
