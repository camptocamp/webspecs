...
case class CswGetRecordsRequest(filter:NodeSeq=Nil,
  ...
  extends AbstractXmlRequest("csw", XmlResponseFactory, CswXmlUtil.getRecordsXml( filter, resultType, outputSchema, startPosition, maxRecords, elementSetName,sortBy)) {
...
case class CswGetByFileId(fileId:String, outputSchema:OutputSchemas.OutputSchema)
  extends AbstractXmlRequest (
    "csw", XmlResponseFactory, CswXmlUtil.getByIdXml(fileId,outputSchema)){
