...
class KeywordsXlinkAddSpec extends GeocatSpecification { def is =
  ...
  lazy val startEditing = {
    ...
    val editValue = StartEditing(MetadataViews.ISOCore, true).execute(Id(id)).value
  ...
  def keywordHtml = {
    startEditing._1.map{n => 
      ...
      val request = GetRequest("metadata.xlink.add", 
          ...
          "href" -> "local://che.keyword.get?thesaurus=external.gemet.inspire-theme&id=http://rdfdata.eionet.europa.eu/inspirethemes/themes/15&locales=fr,en,de,it".encode)
